...
第二类是行为记忆，记住用户行为，从已知的用户行为来帮助决策。最近 Codex 支持通过 agent 来 review
...
通过 AI agent 去分析 report, 并判断应用程序是否足够健康，如果服务器上发现了一些非预期情况就自动总结为 bug report, 之后再由 coding agent 去修复。修复后通过 AI agent 分析我们的定时脚本是否需要优化，去覆盖更多需要观察的部分。
